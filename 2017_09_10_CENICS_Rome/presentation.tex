...
\begin{frame}{Nominal Model}
	...
	\begin{itemize}[<+->]
		...
		\item[] \alert
		{
			...
			\[
				\frac{\delta {h_i}}{\delta t} = \frac{k_{i-1} sign(h_{i-1}, h_i) \sqrt{|h_{i-1}-h_i|}-k_i \sqrt{h_i}}{A_1} 
			\]
...
\begin{frame}{Fault or Attack}
	\begin{itemize}
		\item \alert<+>
		{
			Our system runs over different modes, each of which have a physical
	...
	\begin{itemize}
		...
		\item \alert<+>
		{
			...
			\begin{itemize}
				...
				\item[-]{Not faulty setting: $v_1=v_2=v_3=0.5$}
			\end{itemize}
		}
	\end{itemize}
\end{frame}
